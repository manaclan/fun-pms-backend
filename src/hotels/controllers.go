...
func (controllers Controllers) SearchByKeyword(c *gin.Context) {
	fmt.Println("Controller: Search hotel by keyword")
	...
	code, hotels := controllers.Services.Search(query["word"][0])
	c.JSON(code, gin.H{
		"status": "succeed",
		"hotels": hotels,
	})
